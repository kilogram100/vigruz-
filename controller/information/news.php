...
class ControllerInformationNews extends Controller {
	public function index() {
		...
		foreach ($all_news as $news) {
			$data['all_news'][] = array (
				...
				'image'			=> $this->model_tool_image->resize($news['image'], 150, 100),
				'description' 	=> (strlen(strip_tags(html_entity_decode($news['short_description'], ENT_QUOTES))) > 400 ? substr(strip_tags(html_entity_decode($news['short_description'], ENT_QUOTES)), 0, 400) . '...' : strip_tags(html_entity_decode($news['short_description'], ENT_QUOTES))),
	...
	public function news() {
		...
		if ($news) {
			...
			$data['image'] = $this->model_tool_image->resize($news['image'], 450, 300);
