...
class ControllerProductProduct extends Controller {
	...
	public function index() {
	    ...
	    if(  ( strpos($data['description'], '=== Классификация ===') === false ) 
		AND( strpos($data['description'], '=== Технические характеристики ===') === false ) 
	    ){ }else{
		$data['description'] = '<pre>'. $data['description'] .'</pre>';
			...
			if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                ...
                $data['reserve'] = '';
				if ($product_info['reserve'] > 0) {
                	$data['reserve'] = " (резерв ".$product_info['reserve'].")";
				}
				$data['unit'] = '';
