...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if (isset($this->request->get['sort'])) {
			...
			//$sort = 'p.sort_order'; сортировка по кол-ву
			$sort = 'p.quantity';
		}
		...
		if ($category_info) {
            ...
            $p_path = (string)$this->request->get['path'];
            if ($p_path == "531") {
                $p_path = "";
            }else{
                $mp_path = explode('_', $p_path, 2);
                if ($mp_path[0] == "531") {
                    $p_path = $mp_path[1];
                }
                $p_path = $p_path . '_';
            }
            // вывод категорий картинками
			foreach ($results as $result) {
				...
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'], 160, 160);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', 160, 160);
				}
