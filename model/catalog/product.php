...
class ModelCatalogProduct extends Model {
    ...
    public function filters_rule($filters_ids) {

       // " LEFT JOIN " . DB_PREFIX . "product_filter pf ON (p2c.product_id = pf.product_id) ";

        $sql = "SELECT filter_group_id, filter_id FROM ".DB_PREFIX."filter WHERE filter_id IN (".implode(',',$filters_ids).") ORDER BY filter_group_id";
        ...
        foreach ($query->rows as $result) {
            if( $g_id == $result['filter_group_id'] ) {
                $f_rule .=  " OR ";
            }else{
                $ind +=1;
                $f_rule .=  $fff." INNER JOIN ".DB_PREFIX."product_filter pf".$ind." ON ( (p2c.product_id = pf".$ind.".product_id) AND ( ";
                $g_id = $result['filter_group_id'];
                $fff = ") )";
            }
            $f_rule .=  " pf".$ind.".filter_id= ". $result['filter_id'];
        }

        if (!empty($f_rule)) { $f_rule .=  ") )"; }
        
        return $f_rule ;
/*        
  INNER JOIN oc_product_filter pf1 ON ( (p2c.product_id = pf1.product_id) 
            AND (  pf1.filter_id= 1)) 
  INNER JOIN oc_product_filter pf2 ON ( (p2c.product_id = pf2.product_id) 
            AND (  pf2.filter_id= 5 OR  pf2.filter_id= 4))"
*/
    }
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			...
			if (!empty($data['filter_filter'])) {
                ...
                $sql .= $this->filters_rule($filters) . " LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id)";;
